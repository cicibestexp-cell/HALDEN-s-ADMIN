...
async function submitPaymentModal() {
  ...
  try {
    ...
    if (window.currentPaymentMethod === 'cash') {
      ...
      await updateDoc(resRef, { paymentMethod: 'cash', cashPaymentStatus: 'waiting_for_admin' });
      document.getElementById('pay-selection-area').style.display = 'none';
      ...
      const payload = {
        items: [
          { name: 'Reservation Fee - ' + resData.type, amount: parseInt(resData.paymentAmount) * 100, quantity: 1 }
        ],
        customerInfo: {
...
window.renderCustomerPayments = async function() {
  ...
  list.innerHTML = '<div style="padding:20px; color:var(--text-dim);">Loading payments...</div>';
  
  try {
    let snap = await getDocs(query(collection(window.firebaseDB, 'reservations'), where('email', '==', userEmail)));
    ...
    if (myRes.length === 0) {
      snap = await getDocs(query(collection(window.firebaseDB, 'reservations'), where('customerEmail', '==', userEmail)));
      myRes = snap.docs.map(d => ({id: d.id, ...d.data()})).filter(r => 
        ['approved','confirmed','procuring','preparing','on-going','completed'].includes((r.status||'').toLowerCase())
      );
    }
    ...
    if (myRes.length === 0) {
      list.innerHTML = '<div style="padding:40px; text-align:center; color:var(--text-dim);">No active payments found.</div>';
      return;
    }
    ...
    list.innerHTML = myRes.map(res => {
      ...
      return `
      <div style="background:var(--bg3); border:1px solid var(--border); border-radius:20px; overflow:hidden; margin-bottom:24px; box-shadow:0 8px 32px rgba(0,0,0,0.08);">
        <div style="background:linear-gradient(135deg, var(--brown) 0%, #2d1f0f 100%); padding:24px; color:#fff;">
          <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            <div>
              <div style="font-size:11px; color:var(--gold); font-weight:700; text-transform:uppercase; letter-spacing:1px;">Event Reservation</div>
              <div style="font-size:20px; font-weight:700;">${res.type || 'My Event'}</div>
              <div style="font-size:12px; opacity:0.7;">Date: ${res.date}</div>
            </div>
            <div style="text-align:right;">
              <div style="font-size:11px; opacity:0.7;">Total Amount</div>
              <div style="font-size:24px; font-weight:800; color:var(--gold);">₱${total.toLocaleString()}</div>
            </div>
          </div>
          <div style="margin-top:16px;">
            <div style="height:6px; background:rgba(255,255,255,0.1); border-radius:3px;">
              <div style="height:100%; background:var(--gold); width:${pct}%; border-radius:3px;"></div>
            </div>
          </div>
        </div>
        <div style="padding:20px; display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
          <div style="padding:16px; background:var(--bg2); border-radius:12px;">
            <div style="font-size:12px; color:var(--text-dim);">Settled</div>
            <div style="font-size:18px; font-weight:700;">₱${paid.toLocaleString()}</div>
          </div>
          <div style="padding:16px; background:var(--bg2); border-radius:12px;">
            <div style="font-size:12px; color:var(--text-dim);">Remaining</div>
            <div style="font-size:18px; font-weight:700;">₱${balance.toLocaleString()}</div>
          </div>
        </div>
      </div>`;
    }).join('');
  } catch(e) {
    list.innerHTML = '<div style="color:var(--red);">Error loading payments.</div>';
  }
};
...
function renderProcFood(items, pax, procData) {
  ...
  foodItems.forEach(dish => {
    ...
    html += `
      <div style="background:rgba(255,255,255,0.02); border:1px solid var(--border); border-radius:8px; padding:12px; margin-bottom:10px;">
        <div style="font-weight:700; color:var(--gold); font-size:12px; margin-bottom:6px;">${dish.name}</div>
        <div style="font-size:11px; color:var(--text-dim); line-height:1.5;">
          ${ings.map(i => `• ${i.totalQty} ${i.unit} ${i.name} <span style="float:right; color:var(--gold);">₱${Math.round(i.totalCost).toLocaleString()}</span>`).join('<br>')}
        </div>
      </div>
    `;
  });
  list.innerHTML = html || '<div style="font-size:11px; color:var(--text-dim); padding:10px;">No food items.</div>';
}
...
function renderProcDecor(items, procData) {
  ...
  list.innerHTML = decorItems.map(i => `
    <div style="background:rgba(255,255,255,0.02); border:1px solid var(--border); border-radius:8px; padding:10px; margin-bottom:8px; font-size:11px;">
      <div style="font-weight:700;">${i.name}</div>
      <div style="color:var(--text-dim); margin-top:2px;">Design: <span style="color:var(--gold);">Confirmed</span></div>
    </div>
  `).join('') || '<div style="font-size:11px; color:var(--text-dim); padding:10px;">No decorations.</div>';
}
...
function renderProcPersonnel(items, procData) {
  ...
  if (!list) return;
  list.innerHTML = `
    <div style="background:rgba(196,154,60,0.05); border:1px solid var(--gold); border-radius:8px; padding:10px; margin-bottom:8px; font-size:11px;">
      <div style="font-weight:700; color:var(--gold);">Event Coordinator</div>
      <div style="color:var(--cream); margin-top:4px;">Marie Clara - Hired</div>
    </div>
    <div style="background:rgba(255,255,255,0.02); border:1px solid var(--border); border-radius:8px; padding:10px; margin-bottom:8px; font-size:11px;">
      <div style="font-weight:700;">Service Staff</div>
      <div style="color:var(--text-dim); margin-top:4px;">Allocated: 6 Pax</div>
    </div>
  `;
}

window.renderReservationTimelineView = function() {
  ...
  if (summary) summary.textContent = `Coverage for event on ${res.date}. Preparation started 10 days prior.`;

  const events = [
    { title: 'Event Date: ' + res.type, start: res.date, color: 'var(--gold)', allDay: true },
    { title: 'Menu Finalization', start: '2026-05-05', color: '#2d8a4e', allDay: true },
    { title: 'Equipment Prep', start: '2026-05-08', color: '#d97706', allDay: true },
    { title: 'Staff Huddle', start: '2026-05-12', color: '#8a7a5a', allDay: true }
  ];
